...
if (strcmpi(button, 'Yes'))
    if (noiseFactor < 0.15 && noiseFactor > 0.01)
        % Alternative noise reduction using moving average filter
        windowSize = 5; % Define the window size for the moving average filter
        b = (1/windowSize) * ones(1, windowSize);
        a = 1;
        recoveredSound = filter(b, a, noisySound);

        % Play and visualize the recovered sound
        soundsc(recoveredSound, Fs);
        fourierTransform(Fs, recoveredSound, 'The Recovered Wave File', 'The Recovered Wave FFT');

        % Ask if the user wants to amplify the recovered signal
        promptMessage = sprintf('Do you want to Amplify recovered signal?');
        titleBarCaption = 'Amplify Sound';
        button = questdlg(promptMessage, titleBarCaption, 'Yes', 'No', 'Yes');
        if (strcmpi(button, 'Yes'))
